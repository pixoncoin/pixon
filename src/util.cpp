...
const char * const BITCOIN_CONF_FILENAME = "pixon.conf";
const char * const BITCOIN_PID_FILENAME = "pixond.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "pixon" is a composite category enabling all Pixon-related debug output
            if(ptrCategory->count(string("pixon"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Pixon
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Pixon
    // Mac: ~/Library/Application Support/Pixon
    // Unix: ~/.pixon
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Pixon";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Pixon";
#else
    // Unix
    return pathRet / ".pixon";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty pixon.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override pixon.conf
